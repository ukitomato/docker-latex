FROM alpine:latest

COPY texlive.profile .

ENV PATH /usr/local/texlive/2021/bin/x86_64-linuxmusl:$PATH

RUN apk add --no-cache curl perl fontconfig-dev freetype-dev && \
    apk add --no-cache --virtual .fetch-deps xz tar wget && \
    mkdir /tmp/install-tl-unx && \
    curl -L https://mirror.ctan.org/systems/texlive/tlnet/install-tl-unx.tar.gz | \
    tar -xz -C /tmp/install-tl-unx --strip-components=1 && \
    /tmp/install-tl-unx/install-tl \
    --profile texlive.profile \
    --repository http://ftp.jaist.ac.jp/pub/CTAN/systems/texlive/tlnet/ && \
    tlmgr install xkeyval xstring microtype etoolbox amsmath booktabs caption fancyvrb libertine totpages environ textcase hyperxmp ifmtarg luacode xcolor ncctools float preprint inconsolata newtx fontspec txfonts latexmk comment url siunitx upquote here cmap setspace acmart todonotes && \
    rm -fr /tmp/install-tl-unx && \
    apk del .fetch-deps

WORKDIR /working

COPY precompile/ /working

RUN cd /working && latexmk --lualatex sample-lualatex.tex && rm -rf *

CMD ["/bin/ash"]





